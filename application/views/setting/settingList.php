        ...
        <div class="row">
            <div class="col-lg-2 col-md-3 col-sm-12 uploadsticky">
                ...
                <div class="box box-primary">
                    <div class="box-body text-center">
                        <?php
                        if ($result->result_logo == "") {
                            ?>
                            <img src="<?php echo base_url() ?>uploads/school_content/logo/images.png" class="img-thumbnail img-responsive " alt="" width="" height="">
                            <?php
                        } else {
                            ?>
                            <img src="<?php echo base_url() ?>uploads/school_content/logo/result_logo/<?php echo $result->result_logo; ?>" class="img-responsive" alt="" width="" height="">
                            <?php
                        }
                        ?>
                        <br/>
                        <a href="#result_logo" role="button" class="btn btn-primary btn-sm upload_result_logo" data-toggle="tooltip" title="<?php echo 'Edit Result Logo'; ?>" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> Processing"><i class="fa fa-picture-o"></i> <?php echo 'Edit Result Logo'; ?></a>
                    </div>
                </div>
            </div>
...
<div class="modal fade" id="modal-upload_result_logo" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel"><?php echo 'Edit Result Logo'; ?></h4>
            </div>
            <div class="modal-body upload_logo_body">
                <!-- ==== -->
                <form class="box_upload boxupload has-advanced-upload" method="post" action="<?php echo site_url('schsettings/ajax_resultlogo') ?>" enctype="multipart/form-data">
                    <input value="<?php echo $result->id ?>" type="hidden" name="id" id="id_result_logo"/>
                    <input type="file" name="file" id="file_resultlogo">
                    <!-- Drag and Drop container-->
                    <div class="box__input upload-result_logo_area"  id="uploadresult_logo">
                        <i class="fa fa-download box__icon"></i>
                        <label><strong><?php echo $this->lang->line('choose_a_file'); ?></strong><span class="box__dragndrop"> <?php echo $this->lang->line('or') ?> <span><?php echo $this->lang->line('drag') ?></span><?php echo $this->lang->line('it_here') ?></span>.</label>
...
// set focus when modal is opened
    $('#modal-upload_app_logo').on('shown.bs.modal', function () {
     ...
     $('.upload_result_logo').on('click', function (e) {
        e.preventDefault();
        var $this = $(this);
        logo_type = $this.data('logo_type');

        $this.button('loading');
        $('#modal-upload_result_logo').modal({
            show: true,
            backdrop: 'static',
            keyboard: false
        });
    ...
    // set focus when modal is opened
    $('#modal-upload_result_logo').on('shown.bs.modal', function () {
        $('.upload_result_logo').button('reset');
    });
    ...
    $(function () {
        ...
        // Drag enter
        $('.upload-result_logo_area').on('dragenter', function (e) {
            e.stopPropagation();
            e.preventDefault();
            $("h1").text("Drop");
        });
        ...
        // Drag over
        $('.upload-result_logo_area').on('dragover', function (e) {
            e.stopPropagation();
            e.preventDefault();
            $("h1").text("Drop");
        });
        ...
        // Drop
        $('.upload-result_logo_area').on('drop', function (e) {
            e.stopPropagation();
            e.preventDefault();
            ...
            fd.append('file', file[0]);
            fd.append("id", $('#id_result_logo').val());
            // fd.append("logo_type", logo_type);

            uploadResultLogoData(fd);
        });

        // Open file selector on div click
        $("#uploadresult_logo").click(function () {
            $("#file_resultlogo").click();
        });

        // file selected
        $("#file_resultlogo").change(function () {
            var fd = new FormData();

            var files = $('#file_resultlogo')[0].files[0];
...
// Sending AJAX request and upload file
    ...
    function uploadResultLogoData(formdata) {

        $.ajax({
            url: '<?php echo site_url('schsettings/ajax_resultlogo') ?>',
            type: 'post',
            data: formdata,
            contentType: false,
            processData: false,
            ...
            beforeSend: function () {
                $('#modal-upload_result_logo').addClass('modal_loading');
            },
            success: function (response) {
            ...
            complete: function () {
                $('#modal-upload_result_logo').removeClass('modal_loading');
