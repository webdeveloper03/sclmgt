...
class Smsgateway {
    ...
    public function sendSMS($send_to, $template = '', $detail) 
    {
        ...
        if(!empty($sms_detail)) 
        {            
            $surl = "http://sms.myposapp.com/api/?username=".$sms_detail->sms_config_username."&password=".$sms_detail->sms_config_password."&message=".$msg."&sender=".$sms_detail->sms_config_sender."&mobiles=".$send_to;            

            $rh = curl_init();
            curl_setopt($rh, CURLOPT_URL, $surl);
            curl_setopt($rh, CURLOPT_RETURNTRANSFER, 1);
            $current_output = curl_exec($rh);
            curl_close($rh);

            $current_data = json_decode($current_output,true);

            if(isset($current_data['status']) && $current_data['status'] == 'OK')
            {
                return true;                    
            }            
        }
