...
class Studentfeemaster_model extends MY_Model {
    ...
    {
        $where_student['b.id'] = $data['student_fees_master_id'];
        $student = $this->smsconfig_model->select_student($where_student);
        $student = $student->row();
        ...
        $mode = $data['amount_detail']['payment_mode'];

        if($mode == 'Cash')
        {
            $where_sms['fees_sms_id'] = 1;
        }

        if($mode == 'Cheque')
        {
            $where_sms['fees_sms_id'] = 2;
        }

        if($mode == 'bank_transfer')
        {
            $where_sms['fees_sms_id'] = 3;
        }

        if($mode == 'card')
        {
            $where_sms['fees_sms_id'] = 4;
        }

        $sms_active_result = $this->smsconfig_model->select('fees_sms',$where_sms);
        $sms_active = $sms_active_result->row();

        if($sms_active->fees_sms_status == 1)
        {
            ...
            if($result->balance > 0)
            {
                if($student->mobileno != "")
                {
                    $date     = $data['amount_detail']['date'];
                    $name     = $student->firstname.' '.$student->lastname;
                    $amount   = $data['amount_detail']['amount'];
                    $fee_name = $student->name;
                    
                    //$message = 'The fee payment of '.$amount.' for '.$fee_name.' of the student '.$name.' is paid successfully on '.$date;

                    $message = 'Alert: '.$name.' you have successfully paid N'.$amount.' for '.$fee_name.' on '.$date;

                    $this->smsgateway->sendSMS($student->mobileno, "", ($message));
                }
            }
        }
    }
...
//echo "<pre>"; print_r($end_date); echo "</pre>";die;
    ...
    public function fee_deposit_collections($data) 
    {
        if(!empty($data)) 
        {
            ...
            {
                $where_student['b.id'] = $data[$d_key]['student_fees_master_id'];
                $student = $this->smsconfig_model->select_student($where_student);
                $student = $student->row();                
                ...
                $mode = $data[$d_key]['amount_detail']['payment_mode'];
                ...
                if($sms_active->fees_sms_status == 1)
                {
                    ...
                    if($result->balance > 0)
                    {
                        if($student->mobileno != "")
                        {
                            $date     = $data[$d_key]['amount_detail']['date'];
                            $name     = $student->firstname.' '.$student->lastname;
                            $amount   = $data[$d_key]['amount_detail']['amount'];
                            $fee_name = $student->name;
                            ...
                            $message = 'Alert: '.$name.' you have successfully paid N'.$amount.' for '.$fee_name.' on '.$date.' , Balance '.$total_balance_amount;
