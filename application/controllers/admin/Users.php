...
if (isset($_POST["search"])) {
    ...
    {
        $where['user_id'] = $this->input->post('stud_id');
        $where['role']    = 'student';
        $credentials = $this->broadsheet_model->select('users',$where);
        $data['credentials'] = $credentials->row();

        $this->load->view("admin/users/edit_credentials", $data);
    }

    public function update_credentials()
    {
        $where_check['id !='] = $this->input->post('id');
        $where_check['username'] = $this->input->post('username');
        $check = $this->broadsheet_model->select('users',$where_check);

        if($check->num_rows() > 0)
        {
            $this->session->set_flashdata('error','Username Already Exists');
            redirect('admin/users/logindetailreport');
        }

        $upd['username'] = $this->input->post('username');
        $upd['password'] = $this->input->post('password');

        $where['id'] = $this->input->post('id');
        $add = $this->broadsheet_model->update('users',$upd,$where);
