...
class Payroll extends Admin_Controller {
    ...
    function index() {
        ...
        $where_mode['payment_mode_status'] = 1;
        $data['payment_modes'] = $this->broadsheet_model->select('payment_modes',$where_mode);
    ...
    function paymentSuccess() 
    {
        ...
        else 
        {
            ...
            if($payment_mode == 1 && $result->balance == 0)
            {
                $msg = array(
                    'payment_balance' => 'Message balance zero',
                );
            ...
            else
            {
                ...
                if($this->input->post("paymentcontact") != "") 
                {
                    if($payment_mode == 1)
                    {
                        $payment_sms = $this->broadsheet_model->select('payment_sms');
                        $payment_sms = $payment_sms->row();

                        $message = $amount.'. '.$payment_sms->payment_sms_content.' '.$emp_name;

                        $this->smsgateway->sendSMS($this->input->post("paymentcontact"), "", ($message));
                    }
                }
    ...
    function payslipView() {
        ...
        $modes = $this->broadsheet_model->select('payment_modes');

        foreach($modes->result() as $mode)
        {
            $mode_name[$mode->payment_mode_id] = $mode->payment_mode_name;
        }
