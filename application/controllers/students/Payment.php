        ...
        if (!empty($this->payment_method)) {
            ...
            if ($pay_method->payment_type == "paypal") {
                ...
                if ($pay_method->monnify_api_key == "") {
                    ...
                    $student_detail = $student->row();
                    $student_mobile = $student_detail->mobileno;

                    $page = new stdClass();
                    $page->symbol = $this->setting[0]['currency_symbol'];
                    $page->currency_name = $this->setting[0]['currency'];
                    $params = array(
                        'key' => $pay_method->monnify_api_key,
                        'contract_code' => $pay_method->monnify_contract_code,
                        'sub_account_code' => $pay_method->monnify_sub_account,
                        'fee_percentage' => $pay_method->monnify_fee_percentage,
                        'split_amount' => $pay_method->monnify_split_amount,
                        'sms_enable' => $pay_method->monnify_enable_sms,
                        'commission_amount' => $pay_method->monnify_commission_amount,
                        'sms_content' => $pay_method->monnify_sms_content,
