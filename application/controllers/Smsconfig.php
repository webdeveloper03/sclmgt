...
class Smsconfig extends Admin_Controller {
    ...
    function index()
    {
        ...
        $sms_result = $this->smsconfig_model->select('sms_config');
        $data['smslist'] = $sms_result->row();        
    ...
    public function balance()
    {
        ...
        $this->session->set_userdata('top_menu', 'sms');
        $this->session->set_userdata('sub_menu', 'smsconfig/balance');
        
        $sms_result = $this->smsconfig_model->select('sms_config');
        $sms = $sms_result->row();        

        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL,'http://sms.myposapp.com/api/?username='.$sms->sms_config_username.'&password='.$sms->sms_config_password.'&action=balance');
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('smsconfig/balance', $data);
        $this->load->view('layout/footer', $data);
    }    
    ...
    public function clickatell() 
    {
        ...
        $this->form_validation->set_rules('clickatell_user', '', 'required');
        $this->form_validation->set_rules('clickatell_password', '', 'required');
        $this->form_validation->set_rules('clickatell_sender', '', 'required');
        $this->form_validation->set_rules('clickatell_status', '', 'required');

        if($this->form_validation->run()) 
        {
            $sms_config = $this->smsconfig_model->select('sms_config');

            $ins['sms_config_username']   = $this->input->post('clickatell_user');
            $ins['sms_config_password']   = $this->input->post('clickatell_password');
            $ins['sms_config_sender']     = $this->input->post('clickatell_sender');
            $ins['sms_config_status']     = $this->input->post('clickatell_status');
            $ins['sms_config_updated_on'] = date('Y-m-d H:i:s');
            $ins['sms_config_updated_by'] = $this->session->userdata('admin')['id'];

            if($sms_config->num_rows() == 0)
            {
                $ins['sms_config_added_on'] = date('Y-m-d H:i:s');
                $ins['sms_config_added_by'] = $this->session->userdata('admin')['id'];

                $add = $this->smsconfig_model->insert('sms_config',$ins);
            ...
            {
                $where_sms['sms_config_id'] = 1;
                $add = $this->smsconfig_model->update('sms_config',$ins,$where_sms);
